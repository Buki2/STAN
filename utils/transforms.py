...
def random_affine2(img, mask, targets, targets2_list, ocr_num, degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-2, 2),
                  borderValue=(123.7, 116.3, 103.5), all_bbox=None):
    border = 0  # width of added border (optional)
    height = max(img.shape[0], img.shape[1]) + border * 2
    ...
    if type(targets)==type([1]):
        ...
        for ii in range(all_bbox.shape[0]):
            all_bbox[ii,:] = wrap_points(all_bbox[ii,:], M, height, a)
        return imw, maskw, targets, all_bbox, M
    elif targets is not None:   # enter this branch
        targets = wrap_points(targets, M, height, a)
        for ii in range(ocr_num):
            targets2_list[ii] = wrap_points(targets2_list[ii], M, height, a)
        return imw, maskw, targets, targets2_list, M
    else:
        return imw
